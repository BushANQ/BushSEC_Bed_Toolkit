...
class AdvancedPEInjector(QWidget):
    ...
    def init_ui(self):
        ...
        tab_widget.insertTab(0, analysis_tab, "文件分析")
        ...
        # 设置各选项卡内容
        self.setup_analysis_tab(analysis_tab)  # 添加新设置
    ...
    def setup_analysis_tab(self, tab):
        ...
        select_file_button = QPushButton("浏览...")
        select_file_button.clicked.connect(self.select_analysis_file)
        ...
        """)
        result_layout.addWidget(self.file_info_text)
        ...
        self.analyze_pe = QCheckBox("PE文件结构分析")
        self.analyze_pe.setChecked(True)
        options_layout.addWidget(self.analyze_pe, 0, 0)
        
        self.analyze_strings = QCheckBox("字符串提取")
        self.analyze_strings.setChecked(True)
        options_layout.addWidget(self.analyze_strings, 0, 1)
        
        self.analyze_entropy = QCheckBox("熵值分析")
        self.analyze_entropy.setChecked(True)
        options_layout.addWidget(self.analyze_entropy, 1, 0)
        
        self.analyze_signature = QCheckBox("签名检测")
        self.analyze_signature.setChecked(True)
        options_layout.addWidget(self.analyze_signature, 1, 1)
    ...
    def select_analysis_file(self):
        ...
        if file_name:
            self.analysis_file_input.setText(file_name)
            self.update_status(f"已选择文件: {file_name}")
            
    def analyze_file(self):
        """执行文件分析"""
        file_path = self.analysis_file_input.text()
        ...
        try:
            ...
            file_info.append(f"文件名: {os.path.basename(file_path)}")
            file_info.append(f"大小: {file_stat.st_size:,} 字节")
            file_info.append(f"创建时间: {datetime.fromtimestamp(file_stat.st_ctime)}")
            file_info.append(f"修改时间: {datetime.fromtimestamp(file_stat.st_mtime)}")
            
            # 计算文件哈希
            file_info.append("\n文件哈希:")
            with open(file_path, 'rb') as f:
                content = f.read()
                file_info.append(f"MD5: {hashlib.md5(content).hexdigest()}")
                file_info.append(f"SHA1: {hashlib.sha1(content).hexdigest()}")
                file_info.append(f"SHA256: {hashlib.sha256(content).hexdigest()}")
            
            # PE文件分析
            if self.analyze_pe.isChecked():
                try:
                    pe = pefile.PE(file_path)
                    file_info.append("\nPE文件分析:")
                    file_info.append(f"运行平台: {'64-bit' if pe.PE_TYPE == pefile.OPTIONAL_HEADER_MAGIC_PE_PLUS else '32-bit'}")
                    file_info.append(f"编译时间: {datetime.fromtimestamp(pe.FILE_HEADER.TimeDateStamp)}")
                    ...
                    if hasattr(pe, 'DIRECTORY_ENTRY_IMPORT'):
                        for entry in pe.DIRECTORY_ENTRY_IMPORT:
                            file_info.append(f"\n{entry.dll.decode()}")
                            for imp in entry.imports[:10]:  # 限制显示数量
                                file_info.append(f"  - {imp.name.decode() if imp.name else hex(imp.address)}")
                    ...
                    for section in pe.sections:
                        ...
                        file_info.append(f"{name}: "
                                       f"VirtualAddress={hex(section.VirtualAddress)}, "
            ...
            # 字符串提取
            if self.analyze_strings.isChecked():
                file_info.append("\n字符串分析:")
                ...
                for byte in content:
                    ...
                    elif len(current_string) >= 4:  # 仅保留长度>=4的字符串
                ...
                # 显示一部分提取的字符串
                interesting_strings = [s for s in set(string_list) 
                                    if any(keyword in s.lower() for keyword in 
                                          ['http', 'cmd', 'exe', 'dll', 'sys', 'reg'])]
                
                if interesting_strings:
                    file_info.append("发现可疑字符串:")
                    for s in sorted(interesting_strings)[:20]:  # 限制显示数量
                        file_info.append(f"  - {s}")
                    if len(interesting_strings) > 20:
                        file_info.append("  ...")
            
            # 熵值分析
            if self.analyze_entropy.isChecked():
                with open(file_path, 'rb') as f:
                    data = f.read()
                entropy = self.calculate_entropy(data)
                file_info.append(f"\n文件熵值: {entropy:.2f}")
                if entropy > 7.0:
                    file_info.append("警告: 高熵值通常表明文件可能被加密或压缩")
            
            # 数字签名检测
            if self.analyze_signature.isChecked() and sys.platform == 'win32':
                try:
                    import win32api
                    ...
                    file_info.append("\n数字签名信息:")
                    try:
                        signer_info = win32security.CryptGetMessageSignerInfo(file_path)
                        file_info.append(f"签名者: {signer_info['Signer']}")
                        file_info.append(f"签名时间: {signer_info['SigningTime']}")
                        file_info.append("签名有效")
                    except:
                        file_info.append("未找到有效的数字签名")
                except ImportError:
                    file_info.append("\n无法验证数字签名 (需要pywin32)")
            ...
            self.file_info_text.setText(f"分析过程出错: {str(e)}")
